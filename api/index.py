...
WEB_APP_URL = "https://bekzodmatkarimov7770-cyber.github.io/Ekranchi-shop/market.html?v=pro_v51"
ADMIN_ID = 1758833704
...
def handle_order(m):
    try:
        ...
        t_qty = data.get('total_qty', 0)
        t_sum = data.get('total_sum', 0)
        pt = data.get('price_type', 'Chakana')
        items = data.get('items', [])
        uname = f"@{m.from_user.username}" if m.from_user.username else "-"

        payload_items, items_txt = [], ""
        for i, it in enumerate(items, 1):
            q, p = int(it.get('qty', 1)), int(it.get('price', 0))
            items_txt += f"{i}. <b>{it.get('name')}</b>\n   └ {q} dona × {p:,} = <b>{q*p:,} so'm</b>\n"
